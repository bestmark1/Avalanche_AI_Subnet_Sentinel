...
// ── Internal Source Tracker ─────────────────────────────────────

/**
 * Mutable tracker for a single data source's consecutive failure state.
 * The orchestrator owns these — they are never exposed externally.
 * Each tick, the tracker is read to build a frozen SourceHealth snapshot.
 */
interface SourceTracker {
  ...
  lastData: RpcData | null | NodeMetricsData | null;
}
 ...
 */
export class PollingOrchestrator implements IOrchestrator {
  private readonly provider: IProvider;
  private readonly collector: ICollector;
  private readonly store: IStateStore;
  private readonly logger: ILogger;
  private readonly admin: ISubnetAdmin;
  private readonly pipeline: IAiPipeline;
  ...
  private readonly rpcTracker: SourceTracker = {
  ...
  private readonly metricsTracker: SourceTracker = {
    consecutiveFailures: 0,
    lastSuccessAt: null,
    lastFailureAt: null,
    lastErrorMessage: null,
  ...
  private processRpcResult(
    result: PromiseSettledResult<RpcData>,
    tickLogger: ILogger,
  ): RpcData | null {
    ...
    return this.rpcTracker.lastData as RpcData | null;
  }
  ...
  private processMetricsResult(
    result: PromiseSettledResult<NodeMetricsData>,
    tickLogger: ILogger,
  ): NodeMetricsData | null {
    if (result.status === 'fulfilled') {
    ...
    return this.metricsTracker.lastData as NodeMetricsData | null;
  }
   ...
   *   - < 3 → NONE
   */
  private buildSourceHealth(tracker: SourceTracker): SourceHealth {
    ...
    if (tracker.consecutiveFailures >= FAILURE_THRESHOLDS.ERROR_AFTER) {
      alertSeverity = AlertSeverity.CRITICAL;
    } else if (tracker.consecutiveFailures >= FAILURE_THRESHOLDS.WARN_AFTER) {
  ...
  private logSourceFailure(
    ...
    if (consecutiveFailures >= FAILURE_THRESHOLDS.ERROR_AFTER) {
      tickLogger.error('source_failure_critical', logData);
    } else if (consecutiveFailures >= FAILURE_THRESHOLDS.WARN_AFTER) {
