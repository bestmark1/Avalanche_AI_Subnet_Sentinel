 ...
 */
export class SystemAutoHealer implements IAutoHealer {
   ...
   */
  public async attemptHeal(command: string): Promise<boolean> {
    this.logger.warn('auto_heal_executing', { command });

    return new Promise<boolean>((resolve) => {
      exec(command, { timeout: EXEC_TIMEOUT_MS, maxBuffer: EXEC_MAX_BUFFER }, (error, stdout, stderr) => {
