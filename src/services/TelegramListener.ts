...
// ── Telegram Bot API Shapes (narrow — only the fields we use) ─────────────────

/** Incoming text or voice message from a user or group chat. */
...
/** Top-level update object returned by getUpdates. */
...
interface GetUpdatesResponse {
  readonly ok: boolean;
  readonly result: TelegramUpdate[];
}
...
/**
 * A single button in a ReplyKeyboardMarkup row.
 * When pressed, Telegram sends `text` as a plain text message from the user.
 */
...
/**
 * Persistent reply keyboard displayed at the bottom of the Telegram UI.
 *
 * Properties:
 *   keyboard      — 2-D array; each inner array is one row of buttons.
 *   resize_keyboard — Telegram shrinks the keyboard to fit the button count.
 *   is_persistent — Keyboard stays visible after the user sends a message;
 *                   it does not collapse unless explicitly removed with
 *                   ReplyKeyboardRemove.
 */
interface ReplyKeyboardMarkup {
  readonly keyboard:       ReplyKeyboardButton[][];
 ...
 *   └─────────────────┴──────────────────┘
 */
const MENU_KEYBOARD: ReplyKeyboardMarkup = {
  keyboard: [
...
export class TelegramListener {
  ...
  private readonly transcriber: ITranscriber | undefined;
  private readonly onRestartRequest: (() => Promise<boolean>) | undefined;
  ...
  constructor(
    ...
    onRestartRequest?: () => Promise<boolean>,
  ) {
  ...
  private async pollLoop(): Promise<void> {
    while (this.running) {
      try {
        ...
        if (!this.running) break; // Intentional stop — exit without backoff delay.
   ...
   */
  private async sendMenuGreeting(chatId: number): Promise<void> {
    await this.sendMessageWithKeyboard(
      chatId,
      '✅ <b>Sentinel is ready.</b>',
      MENU_KEYBOARD,
    );
  }
   ...
   */
  private async handleMenuStatus(chatId: number): Promise<void> {
    await this.sendMessage(chatId, this.formatStatusMessage());
  }
   ...
   */
  private async handleMenuBalance(chatId: number): Promise<void> {
    ...
    if (snapshot.walletBalanceAvax === null) {
      ...
      lines.push('');
      lines.push(
        'Set <code>WALLET_ADDRESS</code> in your <code>.env</code> to enable.',
      );
    } else {
      ...
      lines.push(
        `${statusIcon} <b>Balance:</b> <code>${balance.toFixed(4)} AVAX</code>${usdSuffix}`,
      );
   ...
   */
  private async handleMenuAiAnalysis(chatId: number): Promise<void> {
    ...
    if (analysis === null) {
      ...
      lines.push('');
      lines.push(
        'Analysis fires when a metric threshold is breached or every 24 hours.',
      );
      await this.sendMessage(chatId, lines.join('\n'));
      return;
    }
   ...
   */
  private async handleMenuRestart(chatId: number): Promise<void> {
    if (this.onRestartRequest === undefined) {
      await this.sendMessage(
        chatId,
        '⚙️ <b>Auto-Heal Not Configured</b>\n\n' +
    ...
    if (succeeded) {
      ...
      await this.sendMessage(
        chatId,
        '✅ <b>Restart command completed successfully.</b>\n\n' +
   ...
   */
  private async handleVoiceMessage(chatId: number, fileId: string): Promise<void> {
    try {
      ...
      if (transcript === null) {
        await this.sendMessage(
          chatId,
          '🎙️ Could not transcribe the voice message. Please try again or tap 📊 Status.',
        );
        return;
      }
      ...
      await this.sendMessage(
        chatId,
        '⚠️ Failed to process the voice message. Please try again later.',
      );
    }
  }
  ...
  private formatStatusMessage(): string {
    ...
    if (s.rpc !== null) {
      ...
      lines.push(
        `⛽ <b>Priority fee:</b> ${feeGwei !== null ? feeGwei.toFixed(2) : '?'} gwei`,
      );
    ...
    if (s.walletBalanceAvax !== null) {
      ...
      const usdSuffix = s.rpc?.avaxUsdPrice != null
        ...
        : '';
      lines.push(
        `💰 <b>Wallet:</b> ${s.walletBalanceAvax.toFixed(4)} AVAX${usdSuffix}`,
      );
    }
   ...
   */
  private formatAnalysisResult(analysis: AnalysisResult): string[] {
    ...
    if (analysis.analysisType === 'alert') {
      ...
      if (analysis.triggeredBy.length > 0) {
        ...
        lines.push(
          `🔍 <b>Triggered by:</b> ${
            analysis.triggeredBy.map((v) => v.metric).join(', ')
          }`,
        );
      }
    } else {
      ...
      lines.push('');
      lines.push(
        `🗓 <b>Coverage:</b> <code>${analysis.coverageWindow.from}</code> → ` +
    ...
    lines.push(
      `🔢 <b>Tokens:</b> ${analysis.tokenUsage.inputTokens} in / ` +
 ...
 */
function hexWeiToGwei(hexWei: string): number | null {
  try {
