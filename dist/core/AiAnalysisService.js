...
const REPORT_ANALYSIS_TOOL = {
    ...
    description: 'Report a structured health analysis of the Avalanche subnet. ' +
    ...
    input_schema: {
        ...
        properties: {
            ...
            confidence: {
                ...
                description: 'Self-reported confidence. Use "low" when telemetry is limited or ambiguous.',
            },
            // Alert-only fields
            urgency: {
                type: 'integer',
                minimum: 1,
                maximum: 5,
                description: 'Alert urgency (1=monitor, 3=attention required within 1h, 5=immediate action). ' +
            ...
            trends: {
                ...
                items: { type: 'string' },
                maxItems: 5,
                description: 'Up to 5 notable trends observed across the coverage period. ' +
            ...
            forwardRisks: {
                ...
                items: { type: 'string' },
                maxItems: 5,
                description: 'Up to 5 forward-looking risk items for the next 24h. ' +
                    'Required for summary analyses only.',
            },
        },
    },
...
function buildSystemPrompt() {
    return ('You are an expert Avalanche blockchain node operator and SRE. ' +
...
function buildCompactSnapshot(snapshot) {
    ...
    if (snapshot.rpc !== null) {
        lines.push(`RPC block: ${snapshot.rpc.blockNumber}`, `Gas price: ${snapshot.rpc.gasPrice} (wei hex)`, `Max priority fee: ${snapshot.rpc.maxPriorityFeePerGas} (wei hex)`);
        ...
        if (snapshot.rpc.chainlinkUpdatedAt !== null) {
            ...
            if (ageSec > 3600) {
                ...
                lines.push(`Chainlink oracle updated: ${updatedAt}` +
    ...
    if (snapshot.nodeMetrics !== null) {
        lines.push(`CPU usage: ${snapshot.nodeMetrics.cpuUsage.toFixed(1)}%`, `Network latency: ${snapshot.nodeMetrics.networkLatency.toFixed(0)}ms`);
    }
    else {
        lines.push('Node metrics: unavailable (source stale)');
    }
    lines.push(`RPC source: ${snapshot.sources.rpc.status} ` +
        `(${snapshot.sources.rpc.consecutiveFailures} consecutive failures)`, `Metrics source: ${snapshot.sources.nodeMetrics.status} ` +
        `(${snapshot.sources.nodeMetrics.consecutiveFailures} consecutive failures)`);
    return lines.join('\n');
}
function buildViolationsList(violations) {
    return violations
        .map((v) => `  - ${v.metric}: observed=${v.observedValue} threshold=${v.thresholdValue} ` +
        `direction=${v.direction} (active for ${v.ticksActive} tick(s))`)
        .join('\n');
}
function buildAlertUserMessage(ctx, snapshot) {
    return ('ALERT: Threshold violations detected on the Avalanche subnet.\n\n' +
        'Active violations:\n' +
...
function buildSummaryUserMessage(ctx, snapshot) {
    return (`PROACTIVE 24H SUMMARY\nCoverage window: ${ctx.coverageFrom} → ${ctx.coverageTo}\n\n` +
...
function getStringEnum(input, key, allowed) {
    const val = getString(input, key);
    if (!allowed.includes(val)) {
        throw new Error(`Tool response field "${key}" has invalid value "${val}". Allowed: ${allowed.join(', ')}`);
    }
    return val;
}
function getOptionalIntRange(input, key, min, max) {
    const val = input[key];
    if (val === undefined || val === null)
        return undefined;
    if (typeof val !== 'number' || !Number.isInteger(val) || val < min || val > max) {
        throw new Error(`Tool response field "${key}" must be an integer in [${min}, ${max}], got: ${JSON.stringify(val)}`);
    }
    return val;
}
function getOptionalStringArray(input, key) {
    const val = input[key];
    if (val === undefined || val === null)
        return undefined;
    if (!Array.isArray(val) || !val.every((item) => typeof item === 'string')) {
        throw new Error(`Tool response field "${key}" must be an array of strings, got: ${JSON.stringify(val)}`);
    }
    return val;
}
// ── AiAnalysisService ────────────────────────────────────────────────────────
...
class AiAnalysisService {
    ...
    coordinator;
    logger;
    constructor(config, scheduler, store, logger, notifier, coordinator) {
        this.config = config;
     ...
     */
    async processJob(snapshot, context) {
        ...
        try {
            ...
            if (this.notifier !== undefined) {
                void this.notifier.sendAlert(result).catch((err) => {
     ...
     */
    async callLlm(snapshot, context) {
        const isAlert = context.type === 'alert';
        const userMessage = isAlert
            ? buildAlertUserMessage(context, snapshot)
            : buildSummaryUserMessage(context, snapshot);
     ...
     */
    parseToolResponse(response, context) {
        const toolBlock = response.content.find((block) => block.type === 'tool_use' && block.name === 'report_analysis');
        if (toolBlock === undefined) {
            throw new Error(`LLM response missing expected tool_use block "report_analysis". ` +
        ...
        if (context.type === 'alert') {
            ...
            if (urgencyRaw === undefined) {
                throw new Error('Tool response missing required field "urgency" for alert analysis');
            }
            ...
            const result = {
                ...
                tokenUsage,
                urgency,
                triggeredBy: context.violations,
