...
const CPU_REGEX = /^avalanche_node_cpu_usage\s+(?<value>[\d.]+(?:[eE][+-]?\d+)?)/m;
const LATENCY_REGEX = /^avalanche_node_network_latency\s+(?<value>[\d.]+(?:[eE][+-]?\d+)?)/m;
/**
...
class PrometheusMetricsCollector {
     ...
     */
    handleFetchError(error, durationMs) {
        ...
        if (this.isTimeoutError(error)) {
            ...
            throw new SentinelErrors_js_1.CollectorTimeoutError(this.timeoutMs, cause);
        }
        ...
        throw new SentinelErrors_js_1.CollectorError(`Metrics fetch failed: ${message}`, 'http', cause);
    }
     ...
     */
    parseMetrics(body) {
        const cpuUsage = this.extractFloat(body, 'avalanche_node_cpu_usage', CPU_REGEX);
        const networkLatency = this.extractFloat(body, 'avalanche_node_network_latency', LATENCY_REGEX);
     ...
     */
    extractFloat(body, metricName, pattern) {
        const match = pattern.exec(body);
        if (!match?.groups?.['value']) {
            this.logger.warn('metrics_parse_missing', { metricName });
            throw new SentinelErrors_js_1.MetricParseError(metricName);
        }
    ...
    isTimeoutError(error) {
        if (!error || typeof error !== 'object')
            return false;
        const shaped = error;
        return (shaped.code === 'ECONNABORTED' ||
