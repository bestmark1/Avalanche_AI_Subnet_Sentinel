...
const SentinelErrors_js_1 = require("../errors/SentinelErrors.js");
/**
...
class EthersRpcProvider {
    ...
    jitterFn;
    walletAddress;
    constructor(config, logger, jitterFn = () => 0.5 + Math.random() * 0.5, walletAddress = null) {
        this.provider = new ethers_1.JsonRpcProvider(config.endpoint);
    ...
    async getGasMetrics() {
        ...
        try {
            ...
            const gasPrice = await this.fetchWithRetry('eth_gasPrice', [], controller.signal);
            const maxPriorityFeePerGas = await this.fetchWithRetry('eth_maxPriorityFeePerGas', [], controller.signal);
            const blockNumberHex = await this.fetchWithRetry('eth_blockNumber', [], controller.signal);
            ...
            const { avaxUsdPrice, chainlinkUpdatedAt } = await this.fetchChainlinkPrice(controller.signal);
            const rpcData = {
                gasPrice,
            ...
            if (controller.signal.aborted && !(error instanceof SentinelErrors_js_1.ProviderError)) {
                throw new SentinelErrors_js_1.ProviderError(`RPC operation timed out after ${this.timeoutMs}ms`, 0, this.retryCount, error instanceof Error ? error : undefined);
            }
    ...
    async isConnected() {
        try {
            const timeoutPromise = new Promise((_resolve, reject) => {
                setTimeout(() => reject(new Error(`isConnected timed out after ${CONNECTIVITY_TIMEOUT_MS}ms`)), CONNECTIVITY_TIMEOUT_MS);
            });
     ...
     */
    async fetchWalletBalanceAvax(signal) {
        if (this.walletAddress === null)
            return null;
        try {
            const hexWei = await this.fetchWithRetry('eth_getBalance', [this.walletAddress, 'latest'], signal);
     ...
     */
    async fetchChainlinkPrice(signal) {
        const NULL_RESULT = { avaxUsdPrice: null, chainlinkUpdatedAt: null };
        try {
            const hexData = await this.raceAgainstAbort(this.provider.send('eth_call', [
     ...
     */
    async fetchWithRetry(method, params, signal) {
        let lastError;
        for (let attempt = 1; attempt <= this.retryCount; attempt++) {
            // ── Pre-flight: abort check ──
            if (signal.aborted) {
                throw new SentinelErrors_js_1.ProviderError(`RPC call "${method}" aborted (timeout) before attempt ${attempt}`, attempt, this.retryCount, lastError);
            }
            try {
                // ── Execute: race RPC call against abort signal ──
                const result = await this.raceAgainstAbort(this.provider.send(method, params), signal);
                return result;
            }
            catch (error) {
                ...
                if (signal.aborted) {
                    throw new SentinelErrors_js_1.ProviderError(`RPC call "${method}" aborted (timeout) during attempt ${attempt}`, attempt, this.retryCount, lastError);
                }
                ...
                if (!this.isRetryableError(lastError)) {
                    ...
                    throw new SentinelErrors_js_1.ProviderError(`RPC call "${method}" failed with non-retryable error: ${lastError.message}`, attempt, this.retryCount, lastError);
                }
        ...
        throw new SentinelErrors_js_1.ProviderError(`RPC call "${method}" failed after ${this.retryCount} attempts: ${lastError?.message ?? 'Unknown'}`, this.retryCount, this.retryCount, lastError);
    }
     ...
     */
    raceAgainstAbort(promise, signal) {
        if (signal.aborted) {
    ...
    extractHttpStatus(error) {
        ...
        if (typeof shaped.info?.responseStatus === 'number')
            return shaped.info.responseStatus;
        if (typeof shaped.error?.status === 'number')
            return shaped.error.status;
        return null;
    }
}
exports.EthersRpcProvider = EthersRpcProvider;
//# sourceMappingURL=EthersRpcProvider.js.map